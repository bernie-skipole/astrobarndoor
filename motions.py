...
def check_limit(step, sleep, direction, limit_switch):
...
def set_led(on, ledup, leddown, direction):
    ...
    if direction.value():
        ...
        if leddown.value():
            ...
            ledup.value(1)    # and turn on ledup
    else:
        # going down, so the active led is leddown
        led = leddown
        if ledup.value():
            ledup.value(0)    # turn off ledup
            leddown.value(1)  # and turn on leddown
    if on:
        # request a turn on
        if not led.value():
            # its currently off, so turn it on
            led.value(1)
    else:
        # request a turn off
        if led.value():
...
def slow(status, step, sleep, direction, limit_switch):  # status = 0 for stopped, 1 for slow, 2 for fast
    "Sets to moving slow, returns new status"
    if check_limit(step, sleep, direction, limit_switch):
        return 0
...
def stop(status, step, sleep, direction, limit_switch):  # status = 0 for stopped, 1 for slow, 2 for fast
    ...
    if check_limit(step, sleep, direction, limit_switch):
        return 0
    if slow(status, step, sleep, direction, limit_switch):
        # now running slow
        time.sleep(0.2)
        sleep.value(0)
    return 0
...
def fast(status, step, sleep, direction, limit_switch):  # status = 0 for stopped, 1 for slow, 2 for fast
...
def direction(newdirection, status, step, sleep, direction, limit_switch):
    ...
    if status == 2:
        ...
        status = slow(status, step, sleep, direction, limit_switch)
        direction.value(newdirection)
        return fast(status, step, sleep, direction, limit_switch)
